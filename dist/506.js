import "node:module";
import promises, { mkdir, open as promises_open, readFile, stat as promises_stat, unlink, writeFile } from "node:fs/promises";
import * as __rspack_external_node_fs_5ea92f0c from "node:fs";
import { existsSync, readdir, readdirSync, realpath, realpathSync, stat as external_node_fs_stat, statSync } from "node:fs";
import node_path, { basename, dirname, isAbsolute, normalize, posix, relative as external_node_path_relative, resolve as external_node_path_resolve, sep as external_node_path_sep } from "node:path";
import { fileURLToPath } from "url";
import { createRequire } from "module";
import { createServer } from "node:http";
import node_process from "node:process";
import { __webpack_require__ } from "./0~rslib-runtime.js";
import { logger as logger_logger, parsePosix, dirname as pathe_M_eThtNZ_dirname, castArray, isTTY, pathe_M_eThtNZ_resolve, displayPath, color as logger_color } from "./2366.js";
...
const environmentOptionsCommentRE = /@(?:rstest|vitest|jest)-environment-options\s+(.+?)(?:\r?\n|$)/;
const environmentCommentCache = new Map();
const isEnvironmentName = (name)=>SUPPORTED_ENVIRONMENTS.includes(name);
const readFileHead = async (file)=>{
    const handle = await promises_open(file, 'r');
    try {
...
const normalizeOptionsText = (text)=>text.trim().replace(/^\*/, '').trim().replace(/\*\/$/, '').trim();
const isFileNotFoundError = (error)=>error instanceof Error && 'code' in error && 'ENOENT' === error.code;
const parseEnvironmentComment = (code, filePath = '<inline>')=>{
    if (!code.includes('@') || !code.includes('environment')) return null;
    const environmentMatch = environmentCommentRE.exec(code);
    const optionsMatch = environmentOptionsCommentRE.exec(code);
    if (!environmentMatch && !optionsMatch) return null;
    const comment = {};
    const environmentName = environmentMatch?.[1];
    if (environmentName) {
        if (!isEnvironmentName(environmentName)) throw new Error(`Unsupported test environment "${environmentName}" in ${filePath}. Supported environments: ${SUPPORTED_ENVIRONMENTS.join(', ')}.`);
        comment.name = environmentName;
    }
    const optionsText = optionsMatch?.[1];
    if (optionsText) try {
        const parsed = JSON.parse(normalizeOptionsText(optionsText));
        if (!parsed || 'object' != typeof parsed || Array.isArray(parsed)) throw new Error('environment options must be a JSON object');
        comment.options = parsed;
    } catch (error) {
        throw new Error(`Failed to parse test environment options in ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
    }
    return comment;
};
const parseEnvironmentCommentFromFile = async (file)=>{
    const fileStat = await promises_stat(file).catch((error)=>{
        if (isFileNotFoundError(error)) return null;
        throw error;
    });
    if (!fileStat) return null;
    const cached = environmentCommentCache.get(file);
    if (cached?.mtimeMs === fileStat.mtimeMs && cached.size === fileStat.size) return cached.result;
...
const applyEnvironmentComment = (baseEnvironment, comment)=>{
    const name = comment.name ?? baseEnvironment.name;
    const options = name === baseEnvironment.name ? {
...
function killAndWait(child, signal = 'SIGTERM', timeoutMs) {
    if (null !== child.exitCode || null !== child.signalCode) return Promise.resolve();
    return new Promise((resolve)=>{
        ...
        if (!delivered) return void settle();
...
const getNoTestFilesMessage = ({ context, code, defaultMessage })=>{
    if (!context.relatedResolutionEmpty) return defaultMessage;
    if ('changed' === context.relatedMode && !context.relatedFilters?.length) return `No changed files found, exiting with code ${code}.`;
    const filterLabel = 'changed' === context.relatedMode ? 'changed files' : 'related source files';
    return `No test files found for ${filterLabel}, exiting with code ${code}.`;
};
const formatForceRerunTriggerFiles = (files)=>{
    const [firstFile, ...otherFiles] = files;
    if (!firstFile) return 'files';
    if (0 === otherFiles.length) return `file(${firstFile})`;
    const suffix = 1 === otherFiles.length ? 'file' : 'files';
    return `files(${firstFile} and ${otherFiles.length} ${suffix})`;
};
const getForceRerunTriggerMessage = (context)=>`Changed ${formatForceRerunTriggerFiles(context.relatedRerunFiles ?? [])} matched forceRerunTriggers, running all test files.`;
var __require = /* @__PURE__ */ createRequire(import.meta.url);
function cleanPath(path) {
    let normalized = normalize(path);
    if (normalized.length > 1 && normalized[normalized.length - 1] === external_node_path_sep) normalized = normalized.substring(0, normalized.length - 1);
    return normalized;
}
...
function normalizePath(path, options) {
    ...
    if (resolvePaths) path = external_node_path_resolve(path);
...
var Builder = class {
    ...
    options = {
        maxDepth: 1 / 0,
        suppressErrors: true,
        pathSeparator: external_node_path_sep,
        filters: []
    };
    globFunction;
    constructor(options){
...
function getOptions(options) {
    ...
    opts.cwd = (opts.cwd instanceof URL ? fileURLToPath(opts.cwd) : external_node_path_resolve(opts.cwd || process.cwd())).replace(BACKSLASHES, "/");
...
const filterFiles = (testFiles, filters, dir, mode = 'fuzzy')=>{
    if (!filters.length) return 'exact' === mode ? [] : testFiles;
    const fileFilters = 'win32' === process.platform ? filters.map((f)=>f.split(dist_posix.sep).join('/')) : filters;
    if ('exact' === mode) {
        const normalizeExactMatchPath = (filePath)=>{
            const normalizedPath = dist_posix.normalize(filePath);
            return 'win32' === process.platform ? normalizedPath.toLocaleLowerCase() : normalizedPath;
        };
        const exactFilters = new Set(fileFilters.map((filter)=>normalizeExactMatchPath(filter)));
        return testFiles.filter((testFilePath)=>{
            const absolutePath = normalizeExactMatchPath(testFilePath);
            const relativePath = normalizeExactMatchPath(dist_posix.relative(dir, testFilePath));
...
const getTestEntries = async ({ include, exclude, rootPath, projectRoot, fileFilters, fileFilterMode, includeSource })=>{
    const globOptions = {
        cwd: projectRoot,
        absolute: true,
        ignore: exclude,
    ...
    const virtualFiles = include.filter((pattern)=>!isDynamicPattern(pattern)).map((p)=>dist_posix.resolve(projectRoot, p)).filter((abs)=>!existsSync(abs));
    const testFiles = virtualFiles.length ? Array.from(new Set([
        ...globbedFiles,
        ...virtualFiles
    ])) : globbedFiles;
    if (sourceFiles.length) await Promise.all(sourceFiles.map(async (file)=>{
        try {
    ...
    return Object.fromEntries(filterFiles(testFiles, fileFilters, rootPath, fileFilterMode).map((entry)=>{
...
const isSlice = (ev)=>'X' === ev.ph && 'number' == typeof ev.dur;
const WORKER_SLICE_CATS = new Set([
    'phase',
    'suite',
    'case'
]);
const groupByName = (events, matchCat)=>{
    const byName = new Map();
    let total = 0;
    for (const ev of events){
        if (!isSlice(ev) || !matchCat(ev.cat)) continue;
        const cur = byName.get(ev.name) ?? {
            totalUs: 0,
            count: 0
        };
        cur.totalUs += ev.dur;
    ...
    return [
        ...byName.entries()
            ...
            totalUs,
            count,
            pct: 0 === total ? 0 : totalUs / total * 100
        })).sort((a, b)=>b.totalUs - a.totalUs);
};
const summarizeTrace = (events, rootPath)=>{
    const phases = groupByName(events, (cat)=>'phase' === cat);
    const host = groupByName(events, (cat)=>!WORKER_SLICE_CATS.has(cat));
    const fileUs = new Map();
    let phaseTotal = 0;
    for (const ev of events){
        if (!isSlice(ev) || 'phase' !== ev.cat) continue;
        const testPath = 'string' == typeof ev.args?.testPath ? ev.args.testPath : void 0;
        if (testPath) {
    ...
    const files = [
        ...fileUs.entries()
            ...
            totalUs,
            pct: 0 === phaseTotal ? 0 : totalUs / phaseTotal * 100
        })).sort((a, b)=>b.totalUs - a.totalUs);
    const cases = events.filter((ev)=>isSlice(ev) && 'case' === ev.cat).map((ev)=>({
            name: ev.name,
            path: 'string' == typeof ev.args?.testPath ? displayPath(ev.args.testPath, rootPath) : '',
            durUs: ev.dur
...
const truncated = (items, limit, noun)=>{
    const shown = items.slice(0, limit);
    const rest = items.length - shown.length;
    return {
        shown,
        note: rest > 0 ? `… and ${rest} more ${noun}(s)` : void 0
    };
};
const formatTraceSummary = (summary, options = {})=>{
    const { topFiles = 10, topCases = 10 } = options;
    const blocks = [
        `# Trace summary — ${summary.fileCount} test file(s)`
    ];
    const section = (heading, table, note)=>{
        blocks.push(`${heading}\n\n${table}${note ? `\n\n${note}` : ''}`);
    };
    if (summary.phases.length) section('## Phases (aggregate across workers)', mdTable([
        'phase',
        'total',
        '%',
        'count'
    ...
    if (summary.host.length) section('## Host spans', mdTable([
        'span',
        'total',
        '%',
        'count'
    ...
    if (summary.files.length) {
        ...
        section('## Slowest files (by phase wall time)', mdTable([
            'file',
            'total',
            '%'
        ], shown.map((f)=>[
    ...
    if (summary.cases.length) {
        ...
        section('## Slowest test cases', mdTable([
            'case',
            'file',
            'dur'
        ], shown.map((c)=>[
...
const noopTraceSpan = async (_name, _cat, fn)=>fn();
const getTraceOutputPaths = (rootPath)=>{
    const stamp = new Date().toISOString().replace(/[:.]/g, '-').replace('Z', '');
    const stem = pathe_M_eThtNZ_resolve(rootPath, '.rstest', `trace-${stamp}`);
    return {
        tracePath: `${stem}.json`,
        summaryPath: `${stem}.summary.md`
    };
};
const buildTraceFile = (events, rootPath)=>{
    const meta = (name, pid, tid, args)=>({
            name,
            cat: '__metadata',
            ph: 'M',
            ts: 0,
    ...
    for (const ev of events){
        ...
        if (!seenPid.has(ev.pid)) {
            ...
            metadata.push(meta('process_name', ev.pid, ev.tid, {
                name: sharedWorker ? `worker ${ev.pid}` : displayPath(testPath, rootPath)
        ...
        if (!seenThread.has(threadKey)) {
            ...
            metadata.push(meta('thread_name', ev.pid, ev.tid, {
...
const startTraceServer = (initialPath)=>{
    let activePath = initialPath;
    const server = createServer(async (req, res)=>{
        res.setHeader('Access-Control-Allow-Origin', PERFETTO_ORIGIN);
        res.setHeader('Cache-Control', 'no-cache');
        if ('OPTIONS' === req.method) return void res.writeHead(204).end();
        if (req.url !== SERVED_PATH) return void res.writeHead(404).end();
        try {
    ...
    return new Promise((resolve, reject)=>{
        ...
        server.listen(PERFETTO_PORT, '127.0.0.1', ()=>{
            ...
            resolve({
                ...
                close: ()=>new Promise((res)=>stopListening(()=>res())),
...
const createTraceController = (options)=>{
    ...
    const beginRun = ()=>{
        ...
        const events = [];
        const pushHostSlice = async (name, cat, fn, args)=>{
        ...
        return {
            ...
            finalize: async ()=>{
                ...
                await mkdir(pathe_M_eThtNZ_dirname(tracePath), {
                    recursive: true
                });
                await writeFile(tracePath, JSON.stringify(buildTraceFile(events, rootPath)));
                if (lastTracePath && lastTracePath !== tracePath) unlink(lastTracePath).catch(()=>{});
                lastTracePath = tracePath;
                const summaryMarkdown = formatTraceSummary(summarizeTrace(events, rootPath));
                await writeFile(summaryPath, `${summaryMarkdown}\n`);
                if (lastSummaryPath && lastSummaryPath !== summaryPath) unlink(lastSummaryPath).catch(()=>{});
                lastSummaryPath = summaryPath;
                logger_logger.log(`\n${summaryMarkdown}\n`);
                logger_logger.log(logger_color.gray('  Perfetto trace file: '), logger_color.cyan(tracePath));
                logger_logger.log(logger_color.gray('  Trace summary file: '), logger_color.cyan(summaryPath));
                if (!isTTY('stdin')) return void logger_logger.log(logger_color.gray('  Drag the file above into https://ui.perfetto.dev to view.'));
                if (server) server.setActiveTrace(tracePath);
                else try {
                    server = await startTraceServer(tracePath);
                } catch (err) {
                    logger_logger.log(logger_color.yellow(`  Could not start Perfetto helper server: ${err.message}`), logger_color.gray('\n  Drag the file above into https://ui.perfetto.dev to view.'));
                    return;
                }
                logger_logger.log(logger_color.gray('  Open in Perfetto UI: '), logger_color.cyan(server.url));
            }
        };
    };
    const controller = {
        ...
        waitForExit: async ()=>{
            if (!server) return;
            logger_logger.log(logger_color.gray('  Press Ctrl+C to stop the Perfetto helper server and exit.'));
...
export { applyEnvironmentComment, createTraceController, detect, filterProjects, formatTestEntryName, formatTestPath, getForceRerunTriggerMessage, getNoTestFilesMessage, getTestEntries, glob, isDynamicPattern, killAndWait, noopTraceSpan, parseEnvironmentCommentFromFile, prettyTestPath, resolveShardedEntries };
